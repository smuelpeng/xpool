    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            if self.tokenizer is not None:
                ...
                data['frame_caption'] = self.tokenizer(data['frame_caption'], return_tensors='pt', padding=True,
                                                truncation=True)
                
            if isinstance(data['text'], torch.Tensor):
                data['text'] = data['text'].to(self.device)
                data['frame_caption'] = data['frame_caption'].to(self.device)
            else:
                data['text'] = {key: val.to(self.device) for key, val in data['text'].items()}
                data['frame_caption'] = {key: val.to(self.device) for key, val in data['frame_caption'].items()}
            
            data['video'] = data['video'].to(self.device)

            # text_embeds, video_embeds_pooled = self.model(data)
            # text_embeds, video_embeds, video_embeds_pooled 
            # text_features, video_features_pooled, cap_features, frame_features= self.model(data, frame_caption=True)
            text_embeds, video_embeds_pooled, cap_embeds, frame_embeds = self.model(data, frame_caption=True)
